...
class ImageModel {
  ...
  constructor(signaturePath: string) {
    ...
    this.modelPath = `file://src/assets/AppleOrangetensorFlowJS/${this.signature.filename}`;
...
async function main(imgPath: string) {
  ...
  const model = new ImageModel('src/assets/AppleOrangeTensorFlowJS/signature.json');
  await model.load();
  // run the prediction
